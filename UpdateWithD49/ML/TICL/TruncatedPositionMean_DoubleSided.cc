  ...
  */
  ...
  for (int ievent=0; ievent<nEvents; ++ievent)
  {
    ...
    for (unsigned int k=0; k<rechit_energy->size(); k++)
    {
      ...
      //if(rechit.rechitE > 0.05) std::cout << rechit.rechitX << "," << rechit.rechitY << "," << rechit.rechitZ << "," << rechit.rechitTime << std::endl;
    ...
    for (unsigned int j=0; j<rechits.size(); j++)
    {
      ...
      h_reco_gen_X->Fill(toRH[0], rechits.at(j).rechitX);
      h_reco_gen_Y->Fill(toRH[1], rechits.at(j).rechitY);
    ...
    //h_reco_gen_X->Fill(toRH_average_x/nhits, sumPositionAverageX/sumEnergy);
    //h_reco_gen_Y->Fill(toRH_average_y/nhits, sumPositionAverageY/sumEnergy); 
