...
int TruncatedMean_CrudeMethod(std::string infile, std::string outfile, double rhoCut)
{
  ...
  for (int ievent=0; ievent<nEvents; ++ievent)
  {
    ...
    if(recHits.size() > 0) std::cout << "recHits.at(0).recHitSmearedTime = " << recHits.at(0).recHitSmearedTime << std::endl;

    h_nHits_FullSize->Fill(recHits.size());
  
    int recHitsFullSize = recHits.size();
    int removeElementsLow = 0.30*recHits.size();
    ...
    int removeElementsHigh = 0.50*recHitsFullSize;
    ...
    std::cout << "further reduced recHits.size() = " << recHits.size() << std::endl;
