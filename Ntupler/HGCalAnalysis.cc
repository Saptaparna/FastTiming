...
HGCalAnalysis::HGCalAnalysis(const edm::ParameterSet &iConfig)
  ...
  {
    t_->Branch("multiclus_pcaAxisX", &multiclus_pcaAxisX);
    t_->Branch("multiclus_pcaAxisY", &multiclus_pcaAxisY);
    t_->Branch("multiclus_pcaAxisZ", &multiclus_pcaAxisZ);
    t_->Branch("multiclus_pcaPosX", &multiclus_pcaPosX);
    t_->Branch("multiclus_pcaPosY", &multiclus_pcaPosY);
    t_->Branch("multiclus_pcaPosZ", &multiclus_pcaPosZ);
    t_->Branch("multiclus_eigenVal1", &multiclus_eigenVal1);
    t_->Branch("multiclus_eigenVal2", &multiclus_eigenVal2);
    t_->Branch("multiclus_eigenVal3", &multiclus_eigenVal3);
    t_->Branch("multiclus_eigenSig1", &multiclus_eigenSig1);
    t_->Branch("multiclus_eigenSig2", &multiclus_eigenSig2);
    t_->Branch("multiclus_eigenSig3", &multiclus_eigenSig3);
    t_->Branch("multiclus_siguu", &multiclus_siguu);
    t_->Branch("multiclus_sigvv", &multiclus_sigvv);
    t_->Branch("multiclus_sigpp", &multiclus_sigpp);
    t_->Branch("multiclus_sigee", &multiclus_sigee);
  }
...
void HGCalAnalysis::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup) {
  ...
  for (unsigned int i = 0; i < npart; ++i) {
    ...
    if (myTrack.noEndVertex())  // || myTrack.genpartIndex()>=0)
    {
      ...
      //std::cout << "here line 1288" << std::endl;
      for (unsigned il = 0; il < nlayers; ++il) {
        ...
        //std::cout << "xp = " << propCoords.x << std::endl;
        //std::cout << "yp = " << propCoords.y << std::endl;
  ...
  for (std::vector<SimCluster>::const_iterator it_simClus = simClusters->begin();
    ...
    for (std::vector<std::pair<uint32_t, float>>::const_iterator it_haf =
      ...
      cout << "index_simcluster = " << index_simcluster << endl;
      cout << "it_haf->first = " << it_haf->first << endl;
      hits.push_back(it_haf->first);
      fractions.push_back(it_haf->second);
      cout << "it_haf->second = " << it_haf->second << endl;
    ...
    //{
      ...
      std::cout << "hits_indices = " << hits_indices << std::endl;
...
int HGCalAnalysis::fillLayerCluster(const edm::Ptr<reco::CaloCluster> &layerCluster,
  ...
  for (unsigned int j = 0; j < hfsize; j++) {
    ...
    if(storePCAvariables and multiClusterIndex>=0 and fraction>0 && mip > 0)
    {
      ...
      if(pcavars[2]!=0)
      {
        for (int i=0; i<int(hit->energy()); ++i) pca_->AddRow(pcavars);
      }
    }
...
void HGCalAnalysis::fillRecHit(const DetId &detid, const float &fraction, const unsigned int &layer,
  ...
  else detIdToRecHitIndexMap_[detid] = -99999;
}
