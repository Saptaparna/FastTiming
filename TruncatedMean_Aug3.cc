...
int TruncatedMean_Aug3(std::string infile, std::string outfile, double rhoCut, double fractionLow, double fractionHigh)
{
  ...
  for (int ievent=0; ievent<nEvents; ++ievent)
  {
    ...
    for (unsigned int j=0; j<recHits.size(); j++)
    {
      ...
      if(recHits.size() >= 3 and rho<rhoCut) h_deltaTime->Fill(recHits.at(j).recHitDeltaTime);
