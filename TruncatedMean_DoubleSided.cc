...
bool sortRecHitsInDescendingSmearedTime(RecHitInfo recHit1, RecHitInfo recHit2)
{
  return (recHit1.recHitSmearedTime > recHit2.recHitSmearedTime);
}
...
int TruncatedMean_DoubleSided(std::string infile, std::string outfile, double rhoCut, double fractionLow, double fractionHigh)
{
  ...
  for (int ievent=0; ievent<nEvents; ++ievent)
  {
    ...
    for (unsigned int k=0; k<recHit_energy->size(); k++)
    {
      ...
      double rho = sqrt(pow((recHit.recHitX - 114.0), 2) + pow(recHit.recHitY, 2));
      if(recHit_time->at(k) > -2.0 and rho < rhoCut) sumOfEnergy += recHit_energy->at(k);
      if(recHit_time->at(k) > -2.0 and rho < rhoCut) recHits.push_back(recHit);
    }
    ...
    int removeElementsLow = fractionLow*recHits.size();
    int removeElementsHigh = fractionHigh*recHitsFullSize;
    double fraction = (double)(removeElementsLow+removeElementsHigh)/(double)recHitsFullSize;

    if(recHits.size() >= (unsigned int)(3.0/(1-fraction)))
    {
      recHits.erase(recHits.begin(), recHits.begin()+removeElementsLow);

      for(int i=0; i<removeElementsHigh; i++)
      {
        recHits.pop_back();
      }
    }
    else if(recHits.size() > 3)
    {
      int difference = recHits.size() - 3;
    ...
    for (unsigned int j=0; j<recHits.size(); j++)
    {
      ...
      if(recHits.size() >= 3) h_rho->Fill(rho);
      if(recHits.size() >= 3) sumTimeAverage += recHits.at(j).recHitSmearedTime*(1.0/(pow(recHits.at(j).recHitDeltaTime, 2)));
      if(recHits.size() >= 3) sumResolution += 1.0/(pow(recHits.at(j).recHitDeltaTime, 2));
      if(recHits.size() >= 3) nhits++;
      if(recHits.size() >= 3) h_rmsTime->Fill(recHits.at(j).recHitRmsTime);
      if(recHits.size() >= 3) h_soverN->Fill(recHits.at(j).recHitSoverN);
      if(recHits.size() >= 3) h_deltaTime->Fill(recHits.at(j).recHitDeltaTime);
      if(recHits.size() >= 3) h_recHit_Time->Fill(recHits.at(j).recHitSmearedTime);
    }
