...
HGCTimingAnalyzerWithTOAPU::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
        ...
        for (std::vector<CaloParticle>::const_iterator it_caloPart = caloParticle->begin(); it_caloPart != caloParticle->end(); ++it_caloPart)
        {
          ...
          for (CaloParticle::sc_iterator it_sc = simClusterRefVector.begin(); it_sc != simClusterRefVector.end(); ++it_sc) 
          {
            ...
            for (std::vector<std::pair<uint32_t,float> >::const_iterator it_haf = hits_and_fractions.begin(); it_haf != hits_and_fractions.end(); ++it_haf) 
            {
              ...
              //if(bc==0 and ev==0) std::cout << "it_haf->second = " << it_haf->second << " bunch x-ing = " << bc << " event = " << ev << std::endl;
              //else std::cout << "it_haf->second = " << it_haf->second << " bunch x-ing = " << bc << " event = " << ev << std::endl;
              simHitInfo simHitTemp;
              simHitTemp.rawDetId = id;
              simHitTemp.isSignal = 0;
              simHitTemp.isPU = 0;
              simHitTemp.isPUandSignal = 0;
              //implementing Michael's suggestion
              /*if(bc==0 and ev==0)
              {
                simHit.isSignal = 1;
                simHit.isPU = 0;
                simHit.isPUandSignal = 0;
              }
              else if(ev>=1)
              {
                simHit.isPUandSignal = 0;
              ...
              {
                simHit.isPUandSignal = 1;
                simHit.isSignal = 0;
                simHit.isPU = 0;
              }*/
              /*if(bc==0 and ev==0 and it_haf->second==1)//Sapta scheme
              {
                //std::cout << "bc signal = " << bc << endl;
                //std::cout << "ev signal = " << ev << endl;
                simHit.isSignal = 1;
                simHit.isPU = 0;
                simHit.isPUandSignal = 0;
              }
              else if(ev>=1 and it_haf->second==1)
              {
                //std::cout << "bc mixed = " << bc << endl;
                //std::cout << "ev mixed = " << ev << endl;
                simHit.isPUandSignal = 0;
                simHit.isSignal = 0;
                simHit.isPU = 1;
              }
              else //if(it_haf->second<1.0)
              {
                //std::cout << "bc pu = " << bc << endl;
                //std::cout << "ev pu = " << ev << endl;
                simHit.isPUandSignal = 1;
                simHit.isSignal = 0;
                simHit.isPU = 0;
              }*/
              if(bc==0 and ev==0 and it_haf->second==1)//Arabella Scheme implemented earlier 
              {
                simHitTemp.isSignal = 1;
                simHitTemp.isPU = 0;
                simHitTemp.isPUandSignal = 0;
              }
              else if(bc==0 and ev==0 and it_haf->second<1) 
              {
                simHitTemp.isPUandSignal = 1;
                simHitTemp.isSignal = 0;
                simHitTemp.isPU = 0;
              }
              else
	...
	for(unsigned int l=0; l<pfCluster->size(); l++) // Iterating over sim clusters
	{
	  ...
	  for(unsigned ifrac = 0; ifrac < pfCluster->at(l).hitsAndFractions().size(); ifrac++)
	  {
	    ...
	    if(hgcHitEE != srcRecHitEE->end())//check every failure case
	    {
	      const HGCalDetId detId_recHit_ee = hgcHitEE->detid();
              if(id==detId_recHit_ee.rawId())// and hgcHitEE->time() > 0)
	      {
                auto mapSim = mapSimHits.find(id);
                if(mapSim!=mapSimHits.end())
                {
                  simHitInfo simHitEE = mapSim->second;
		  nhits_ee++;
		  const GlobalPoint& recHitPos_ee = geoHandle_ee->getPosition(id); 
	          recHit_isSignal.push_back(simHitEE.isSignal);
                  recHit_isPU.push_back(simHitEE.isPU);
                  recHit_isSignalAndPU.push_back(simHitEE.isPUandSignal);
	    ...
	    if(hgcHitHEF != srcRecHitHEF->end())
	    {
              ...
              if(id==detId_recHit_hef.rawId())// and hgcHitHEF->time() > 0)
	      {
	        auto mapSim = mapSimHits.find(id);
                if(mapSim!=mapSimHits.end())
                {
                  simHitInfo simHitHEF = mapSim->second;	    
                  nhits_hf++;
		  ...
		  unsigned int layer = recHitTools.getLayerWithOffset(id);//alternate way to access the layer
	          recHit_isSignal.push_back(simHitHEF.isSignal);
                  recHit_isPU.push_back(simHitHEF.isPU);
        	  recHit_isSignalAndPU.push_back(simHitHEF.isPUandSignal);
...
void HGCTimingAnalyzerWithTOAPU::beginJob()
{
	...
	branch_=tree_->Branch("recHit_isPU", &recHit_isPU);
        ...
        branch_=tree_->Branch("recHit_isSignalAndPU", &recHit_isSignalAndPU);
